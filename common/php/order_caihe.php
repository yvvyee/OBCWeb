...
function orderCaihe() {
    ...
    foreach ($items as $i => $item) {
        if ($i == 0) { // 최초에 테이블 헤더 세팅
            ...
            $cells = $cells . $cell;
            $cell = sprintf($fmt_td[true], 'th', '', $translate['bugoushu']);
            $cells = $cells . $cell;
        ...
        $query = "SELECT sum(qty) FROM material WHERE item='$item[0]' AND class='彩盒';";        // caihe material
        ...
        $chengpin = mysqli_fetch_array(mysqli_query($conn, $query))[0];
...
//        $chuku = mysqli_fetch_array(mysqli_query($conn, $query))[0];
        ...
        $sum = $qtyOrder - intval($caihe) - $qtyChengpin;

        $cell = sprintf($fmt_td[true], 'td', 'item', $item[0]);
        $cells = $cells . $cell;

        $cell = sprintf($fmt_td['attr'], 'td', "name='qty'; style='text-align: right'", $caihe);
        $cells = $cells . $cell;
